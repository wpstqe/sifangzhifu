...
class Adazfb extends Pay
{
    ...
    public function Pay($array)
    {
        ...
        $payment_params = [
            ...
            'notify_url' => 'http://' . $_SERVER['HTTP_HOST'] . url("/pay/adazfb/notify"),
            'callback_url' => 'http://' . $_SERVER['HTTP_HOST'] . url("/pay/adazfb/callback"),
        ];
        $data = new Payment();
        $data->create($payment_params);
        $result = $data->result;
        if (!isset($result[0]) || $result[0] != 200) {
            $data = json_decode($result[1], true);
            $info = json_decode($data['data'], true);
            $msg = isset($info['error_msg']) ? $info['error_msg'] : '请求出错';
            Db::name('channel_account')->where(['id' => $return['channelaccountid']])->update(['status' => 0]);
            $this->showmessage($msg);
        }
        $data = json_decode($result[1], true);
        $payData = json_decode($data['data'], true);
        if (isset($payData['expend']['pay_info'])) {
            $h5Url = $payData['expend']['pay_info'];
        } else {
            $this->showmessage('支付连接请求失败');
        }
        $data = ['id' => $return['id'], 'url' => $h5Url];
        $h5Url = 'http://' . $_SERVER['HTTP_HOST'] . url("/index/index/change", ['code' => base64_encode(json_encode($data))]);
...
//        echo $this->fetch();die;
    ...
    public function notify()
    {
        ...
        file_put_contents('2.txt', json_encode($ddd) . PHP_EOL, FILE_APPEND);
